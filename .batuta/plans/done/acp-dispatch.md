...
## Tasks
- [x] 1. Introduce a backend seam without changing CLI execution — backend/high
...
- [x] 2. Add bounded receipts and honest optional usage — backend/medium
      Depends on: 1
...
- [x] 3. Implement bounded ACP JSON-RPC communication — backend/high
...
- [x] 4. Implement configured ACP task sessions and result mapping — backend/high
      Depends on: 1, 2, 3
...
- [x] 5. Enforce ACP permission outcomes without implicit approval — backend/high
      Depends on: 4
...
- [x] 6. Verify cancellation and owned subprocess shutdown — backend/high
      Depends on: 4
...
- [x] 7. Select qualified ACP backends while preserving CLI adapters — backend/high
      Depends on: 4, 5, 6
...
- [x] 8. Wire optional ACP into attempts and independent verification — backend/high
      Depends on: 7
...
- [x] 9. Persist dispatch identity and prevent uncertain replay — backend/high
      Depends on: 8
...
- [x] 10. Expose bounded dispatch for interactive skills — backend/high
      Depends on: 7, 9
...
- [x] 11. Document compatibility and measure transport outputs — docs/medium
      Depends on: 10
