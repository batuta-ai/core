...
- [x] 2. No verifier session on an attempt already rejected by tests, scope or a proof — backend/medium
      Depends on: 1
      Scope: loop/attempt.go, loop/verifier_policy_test.go, loop/attempt_test.go, docs/loop.md
...
**Task 2.** Decide the skip after `gates.Tests`, `gates.Scope` and `gates.Proofs` have run and before `r.verify`. The skip reason lists the failing gate names (`tests`, `scope`, `proof N`). The attempt is already failing, so `Decide()` is unaffected; do not change `Decide`. Scope widened on 2026-09-23 after delivery `verifier-coverage-20260923-021113` stopped on it: `TestACPAttemptHonorsIndependentGates` in `loop/attempt_test.go` (~824) asserts `report.Verifier != nil` for every gate, including the tests, scope and proof failures this task skips; change only that assertion so those subtests expect `Verifier` nil while the verifier subtests keep it.
