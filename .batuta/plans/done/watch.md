...
- [x] 3. Journal task_progress records and put the protocol in the brief — backend/high → codex/gpt-6-astra
      Depends on: 2
...
**Sandbox note for executors (added 2026-09-06 after task 3).** The codex sandbox cannot write to Go's default build cache or to the user cache dir where `integration` keeps its preflight scratch. Run tests as `GOCACHE=/private/tmp/batuta-watch-go-cache HOME=/private/tmp/batuta-watch-home go test ./loop -run <Test>` (same two variables for `go test ./...` and `go build`). A `create preflight scratch failed` or `operation not permitted` on the cache is the sandbox, not the code, and never a reason to stop or to edit `integration/`. A red test you just wrote is the expected first step of test-first work, not a failed command under stop condition 2.
